...
const userSchema = z.object({
  ...
  role: z.enum([UserRole.SYSTEM_ADMIN, UserRole.COMPANY_ADMIN, UserRole.USER], {
...
interface UserFormProps {
  onSuccess: () => void;
  onCancel: () => void;
  user?: User | null;  // Düzenlenecek kullanıcı
}
...
export default function UserForm({ onSuccess, onCancel, user }: UserFormProps) {
  ...
  // Şirketleri getir - sadece SystemAdmin için
  useEffect(() => {
    const fetchCompanies = async () => {
      if (isSystemAdmin) {
        try {
          ...
          console.error('Şirketler getirilemedi:', err);
        }
      }
    };
  ...
  const onSubmit = async (data: UserFormData) => {
    ...
    try {
      if (isEditMode && user) {
        ...
        // SystemAdmin ise ve şirket seçildiyse şirket ID'sini ekle
        if (isSystemAdmin && data.companyId) {
          updateData.companyId = data.companyId === 'null' ? null : data.companyId;
        }
        ...
        const createData: any = {
          ...
          firstName: data.firstName,
          lastName: data.lastName,
          password: data.password || '', // Burada varsayılan olarak boş string kullanabilirsiniz
          role: data.role,
        };

        // SystemAdmin ise ve şirket seçildiyse şirket ID'sini ekle
        if (isSystemAdmin && data.companyId) {
          createData.companyId = data.companyId === 'null' ? null : data.companyId;
        }

        await usersApi.adminCreate(createData);
      }
      onSuccess();
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div className="col-span-2 sm:col-span-1">
          ...
          <div className="mt-1 relative rounded-md shadow-sm">
            ...
            >
              ...
              <option value={UserRole.SYSTEM_ADMIN}>Sistem Yöneticisi</option>
            </select>
        ...
        {/* Şirket - Sadece SystemAdmin için göster */}
        {isSystemAdmin && (
          <div className="col-span-2">
            <label htmlFor="companyId" className="block text-sm font-medium text-gray-700">
              Şirket {selectedRole !== UserRole.SYSTEM_ADMIN ? '*' : '(Opsiyonel)'}
            </label>
            <div className="mt-1 relative rounded-md shadow-sm">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              <select
                id="companyId"
                className={`${inputClassName(!!errors.companyId)} appearance-none`}
                {...register('companyId')}
              >
                <option value="">Şirket Seçin{selectedRole === UserRole.SYSTEM_ADMIN ? ' (Opsiyonel)' : ''}</option>
                {selectedRole === UserRole.SYSTEM_ADMIN && (
                  <option value="null">Şirket Yok</option>
                )}
            ...
            {selectedRole !== UserRole.SYSTEM_ADMIN && (
              <p className="mt-1 text-xs text-gray-500">
                {selectedRole === UserRole.COMPANY_ADMIN 
                  ? 'Şirket yöneticisi için bir şirket seçilmelidir' 
                  : 'Normal kullanıcı için bir şirket seçilmelidir'}
              </p>
            )}
          </div>
        )}
